...
import { service } from './AdminTip.service';


import { authenticationService } from '../../../_services';
import { history, Role } from '../../../_helpers';
class AdminTips extends React.Component {
    constructor(props) {
        super(props);
    ...
    activate(name, data) {
        service.activateById(data._id).then(tips => {
            this.getMetals();
            this.modelToggle(name)
        });
    }
    deleteMetals() {
                                ...
                                <Table>
                                    ...
                                    <tbody>
                                        {this.state.tips.map((d, i) =>
                                            <tr key={d.id}>
                                                <th scope="row">{i + 1}</th>
                                                <td>{d.metal_id.name === null? 'N/A': d.metal_id.name}</td>
                                                <td>{d.name}</td>
                                                <td>{d.tip_one}</td>
                                                <td>{d.tip_two}</td>
                                                <td>{d.tip_three}</td>
                                                ...
                                                <td>
                                                    <Button onClick={this.activate.bind(this, 'comEdit', d)} outline color="info" size="sm"><i className="fa fa-pencil-square-o" aria-hidden="true"></i> Approve</Button>{' '}
